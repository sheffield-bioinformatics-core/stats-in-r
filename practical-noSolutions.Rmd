...
We can now use the `t.test` function to perform an *independent* test. The first argument to `t.test` is the *R formula* notation for the test being performed. It allows us to define which columns in our dataset are the numeric and grouping variables in the test.
...
```{r}
diet_melt <- diet %>% select(initial.weight, final.weight, mid.weight) %>% 
  reshape2::melt()
```

...
In base R, we can extract a column from a table using the `$` notation. With tab-completion the column can be selected without introducing any typos. The `count` function can then be used directly to give a tabulation of the values in the column.
...
We need to re-format the data slightly
...
And now remove the `Treatment` column so that it only have numeric data
...
```{r}
gghistogram(df1,x="Value",facet.by = "Var")
```
...
```{r}
ggdensity(df1, x="Value",facet.by = "Var")
```
...
```{r}
gghistogram(df1,x="Value",facet.by = "Var",add_density = TRUE)
```
...
```{r}
gghistogram(df1,x="Value",facet.by = "Var",add_density = TRUE,y="..density..")
```

We can add the *standard* normal curve with the following:-

```{r}
gghistogram(df1,x="Value",facet.by = "Var",add_density = TRUE,y="..density..") + stat_overlay_normal_density(col="steelblue")
```
...
diet %>% 
  select(contains("weight")) %>% 
  mutate(loss = final.weight - initial.weight) %>% 
  t_test(loss ~ 1,alternative = "less")
```

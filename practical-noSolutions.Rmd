...
In base R, we can extract a column from a table using the `$` notation. With tab-completion the column can be selected without introducing any typos. The `table` function can then be used directly to give a tabulation of the values in the column.
...
```{r}
# Use of plotting in base R for convenience
plot(t2)
barplot(t2)
...
We get a significant result! However, if we look at the description for `t.test` it is testing against a population mean of $0$. It is no surprise that we get a significant result! By changing the `mu` argument we perform the desired test.
...
## The Independent t test.......TWO independent groups 

Lets consider that we want to compare whether males or females lost more weight during the trial. Here we have two groups and these can be treated as *independent* variables as different patients belong to the two groups.
...
We can use the `t.test` function to perform an *independent* test, although we need to use the function in a slightly different way. If we recall how we performed the test on initial and final weights, both `final.weight` and `initial.weight` were existing columns in the data

```{r eval=FALSE}
t.test(diet$final.weight, diet$initial.weight,alternative = "less")

## General usage to compare variables in columns X and Y
t.test(diet$X, diet$Y)

```

For the test we propose (males vs females) we do not have corresponding columns for male and female weights. Instead, we have a column which tells us the gender of each individual. For cases such as this we can employ the formula s

```{r eval=FALSE}
## Here, X is a numeric variable and Group contains grouping information
t.test(diet$X ~ diet$Group)
## an alternative with less to type
t.test(X ~ Group, data = diet)
...
Let's revist our dataset. Here, there is a midpoint weight measurement, so we have repeated measures.

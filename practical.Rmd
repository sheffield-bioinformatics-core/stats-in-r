...
Biostat1 <- readxl::readxl(file = "clipboard",header=TRUE,sep="\t")
...
```{r}
RCC <- readxl::read_xlsx("data/RCC2.xlsx")
RCC
```

```{r}
ggplot(RCC, aes(x = gender, y = E2F3)) + geom_violin() + geom_boxplot()
```

```{r}
t.test(E2F3~gender, data=RCC,var.equal=TRUE)
```

```{r}
ggplot(RCC, aes(x = Side, y = ANXA)) + geom_violin() + geom_boxplot()
wilcox.test(ANXA ~ Side, data = RCC)
```

```{r}
ggplot(RCC, aes(x = Grade, y = E2F3)) + geom_violin() + geom_boxplot()
```

```{r}
summary(aov(E2F3 ~ Grade,data=RCC))
TukeyHSD(aov(E2F3 ~ Grade,data=RCC))
```


```{r}
RCC <- RCC %>% mutate(mirDiff = `mir499 post-ttt`-`mir499 pre-ttt`)

ggplot(RCC, aes(x="",y=mirDiff)) + geom_boxplot()
wilcox.test(RCC$`mir499 post-ttt`,RCC$`mir499 pre-ttt`,paired=TRUE,alternative = "greater")
```

```{r}
ggplot(RCC,aes(x = Grade,y = DFFA)) + geom_violin()+geom_boxplot()
```

```{r}
kruskal.test(DFFA~as.factor(Grade),data=RCC)
```
...
##2.ANXA expression levels vary between unilateral and bilateral tumors 
#check normality
ggplot(RCC,aes(sample=ANXA)) + geom_qq() + geom_qq_line()+facet_wrap(~Side) #by qqplot
...
shapiro.test(RCC$mir499.post.ttt)
#not normal
...
```{r}
ggplot(RCC,aes(x="",y=`mir499 pre-ttt`)) + geom_boxplot()
```

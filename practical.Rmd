...
plot(t2)
## add the legend.text=TRUE argument to produce a legend
barplot(t2,legend.text = TRUE)
barplot(t2, beside=TRUE,legend.text = TRUE)
```
...
```{r}
ggplot(df1, aes(x=x)) + geom_histogram(aes(y=..density..)) + geom_density(col="blue")
```
...
```{r eval=FALSE}
ggplot(diet, aes(x = BMI))+ geom_histogram(aes(y=..density..)) + geom_density(col="blue")

ggplot(diet, aes(sample=BMI)) + geom_qq() + geom_qq_line()

shapiro.test(diet$BMI)
```
...
We get a significant result! However, if we look at the description for `t.test` it is testing against a population mean of $0$. No wonder that we get a significant result. By changing the `mu` argument we perform the desired test. By changing the `mu` argument we can perform a test to see if the people in the study are overweight to begin with (using 25 as the population or known mean).
...
Back to our dataset, we might wonder if there is actually any effect due to diet. To perform this test we can compare two columns from the `diet` data frame directly using the `$` notation.

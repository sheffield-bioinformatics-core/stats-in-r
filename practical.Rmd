...
We can visualise these counts as a *barplot*
...
```{r}
ggplot(Arthritis, aes(x = Sex,fill=Improved)) + geom_bar(position="dodge")
```
...
With `group_by` we can use two variables to summarise by. The order in which they are specified will dictate how the proportions are calculated. If we simply want the proportions over the whole dataset, we could divide by the total number of rows
...
```
 ...
 In the below example, proportions are defined in terms of Sex first, and then Improved.
...
The `chisq.test` function will provide a p-value that we can use to assess significance. However, we can only use columns with numerical data so have to exclude the Treatment column.
...
The result of the test is displayed to the screen. If we want extra information we can save the result as a variable and extract the relevant information with the `$` syntax.


```{r}
res <- group_by(Arthritis, Improved, Treatment) %>% 
  summarise(N = n()) %>% 
  spread(Improved, N) %>%
  select(-Treatment) %>% 
  chisq.test()
...
- Read the excel file called `EX Biostat P2.xlsx` in the data folder into R 

```{r eval=FALSE}
tab <- readxl::read_xlsx("data/EX biostat P2.xlsx")
```

- Determine the proportion of females with Grade III tumor in the **total sample** 

- Determine the proportion of Grade III tumors **within females** only

- Use the appropriate test to check if the tumor grade depends on the gender 

- Use the appropriate test to check if the effective drug type depends on the gender 
...
group_by(tab, TumorGrade, Gender) %>% 
  summarise(N=n()) %>% 
      mutate(prop = N / 130) 
#2)
group_by(tab, Gender,TumorGrade) %>% 
  summarise(N=n()) %>% 
    mutate(prop = prop.table(N))
...
group_by(tab, TumorGrade,Gender) %>% 
  summarise(N = n()) %>% 
  tidyr::spread(TumorGrade,N) %>% 
  select(-Gender) %>% 
  chisq.test

#4)
res <- group_by(tab, EffectiveDrug,Gender) %>% 
  summarise(N = n()) %>% 
  tidyr::spread(EffectiveDrug,N) %>% 
  select(-Gender) %>% 
  chisq.test
```
...
5. PDC4, DFFA and ANXA are expressed differently in a patient with RCC  

6. MALAT1 is upregulated (higher) in RCC patients compared to E2F3  

...
- Read the excel file called ‘EX Biostat P1’ into R 
- Identify if the age and hospitalization days are normally distributed
- Use the appropriate descriptive statistics [mean±SD or median (IQ range)] for each variable 
...
In this part we will show how to perform tests to compare 2 (or more) continuous variables. The dataset describes individuals that have been following different diets and their age and gender.
...
For analysis purposes, it will be useful to add an extra column to indicate how much weight each person has lost.
...
**Discussion: Discuss with your neighbours what hypotheses you could test with these data?**

N.B. in practice we would form our hypotheses before collecting the data!
</div>
...
The output of the test is quite informative as it suggest that testing has been performed on sets of differences. In actual fact, this is the same as performing something called a *one-sample test* on the differences we have already calculated:-
...
Data that do not meet the assumptions of ANOVA (e.g. normality) can be tested using a non-parametric alternative. The *Kruskal-Wallis* test if derived from the one-way ANOVA, but uses ranks rather than actual observations. It is also the extension of the Mann-Whitney U test to greater than two groups. 

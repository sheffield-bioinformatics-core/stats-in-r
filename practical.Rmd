...
An intuitive way of showing the test result on the plot is using `stat_compare_means` which will automatically add the p-value to a boxplot.
...
We can use the `t_test` function to perform an *independant* test. As before, the first argument to `t_test` is the *R formula* notation for the test being performed. However, this time we do not need the `pivot_longer` function as we already the `gender` and `weight.loss` variable in our data frame
...
**Note that the code is included from the older version of the course (using base R and ggplot2) as well as rstatix and ggpubr to check we get the same answers**

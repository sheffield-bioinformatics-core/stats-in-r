...
The chi-squared test is most suited to large datasets. As a general rule, the chi-squared test is appropriate if at least 80% of the cells have an expected frequency of 5 or greater. In addition, none of the cells should have an expected frequency less than 1. If the expected values are very small, categories may be combined (if it makes sense to do so) to create fewer larger categories. 
...
- Read the excel file called ‘EX Biostat P1.xlsx’ from the `data` folder into R 
- Identify if the age and hospitalization days are normally distributed
- Calculate the appropriate descriptive statistics [mean±SD or median (IQ range)] for each variable 
</div>
...
In this part we will show how to perform tests to compare 1, 2 (or more) continuous variables. The dataset describes individuals that have been following different diets and their age and gender. The main goal of interest is to determine which of three competing diet regimes results in the greatest weight loss.
...
The one-sample t-test is implemented in the function `t.test`. The only input we need to specify is a *vector* of numeric values and it will perform a statistical test
...
This test we assumed no relationship between the values being tested. i.e. a person in the study cannot be recorded as both male or female. 
...
There is no difference in weight loss between males and females. However, this is not taking into account that there are different diets and some may work better depending on gender (or indeed age). If we attempt to test the difference between diets, the following happens:-
...
At this point we could be about to recommend diet C to those that wish to lose weight. However, are there any other factors in the data that we should be considering? With `ggplot2` we can quite easily visualise the effects of multiple factors on the data. Lets add both gender and diet type into the plot.
...
The excel file ‘RCC’ contains data about the expression levels of some genes in patients with renal cell carcinoma. In your study, you put the following hypotheses.
...
3. H19 levels differ in the same individual at time of diagnosis, after giving one course of chemotherapy and after giving a second course. 

4. Individuals with RCC grade II have different levels of E2F3 than those with grade III or IV 

5. The mean value of miR499 decreases significantly after treatment

6. PDC4, DFFA and ANXA are expressed differently in a patient with RCC  

7. MALAT1 is upregulated (higher) in RCC patients compared to E2F3  

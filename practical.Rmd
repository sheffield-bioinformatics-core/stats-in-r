...
6.	DFFA is higher in patients with grade IV tumors
</div>
...
##1-Read the excel file called 'EX Biostat P1' into R 
Biostat1 <- read.table(file = "clipboard",header=TRUE,sep="\t")
...
```{r eval=FALSE, echo=FALSE}
##1-Read the excel file called 'EX Biostat P2' into R 
Biostat2 <- read.table(file="clipboard",header = T,sep="\t")

##2-Identify if the age and hospitalization days are normally distributed
library(ggplot2)
ggplot(Biostat2, aes(x=Age)) + geom_histogram(aes(y=..density..)) + geom_density()
...
```{r eval=FALSE, echo=FALSE}
RCC2 <- read.table(file = "clipboard",header=TRUE,sep="\t")
